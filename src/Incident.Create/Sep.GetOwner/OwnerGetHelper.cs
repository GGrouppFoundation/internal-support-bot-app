...
internal static class OwnerGetHelper
{
    ...
    internal static ValueTask<Result<LookupValueSetOption, BotFlowFailure>> SearchUsersOrFailureAsync(
        this IUserSetSearchFunc UserSetSearchFunc,
        ...
        .HandleCancellation()
        .Pipe(
            flowState => new UserSetSearchIn(
                searchText: seachText,
                MaxUserSetCount))
        .PipeValue(
            UserSetSearchFunc.InvokeAsync)
        .MapFailure(
            MapToFlowFailure)
        .MapSuccess(
            static @out => new LookupValueSetOption(
                items: @out.Users.Select(MapUserItem).ToArray(),
                choiceText: @out.Users.Any() ? ChooseOwnerMessage : UnsuccessfulSearchResultText));

    internal static string CreateResultMessage(IChatFlowContext<IncidentCreateFlowState> context, LookupValue userValue)
        =>
        $"Ответственный: {context.EncodeTextWithStyle(userValue.Name, BotTextStyle.Bold)}";
